...
export async function getServerSideProps(context) {
  ...
  const res = await axios.get(`${baseURL}/${currentId}`);
  const post = res.data;
  console.log(post);
  return { props: { post } };
}
...
const Edit = ({ post }) => {
  ...
  const [content, setContent] = useState(`${post.detail}`);
  const [progress, setProgress] = useState(`${post.status}`);
  ...
  return (
    <div className="w-2/5 bg-teal-100 rounded-md  mx-auto p-9 my-20">
      <div className="flex flex-col  align-middle">
        ...
        <button className="bg-white h-10 w-14 border rounded-lg mx-auto mt-7" onClick={changeButton}>
          修正
        </button>
